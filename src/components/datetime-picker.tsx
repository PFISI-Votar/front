...
>(
  (
    { value, onChange, onBlur, disabled = false, minDate, className, ...props },
    ref
  ) => {
    const { date, hours, minutes } = parseDateTimeValue(value)

    const emitChange = React.useCallback(
      (nextDate: Date | undefined, nextHours: string, nextMinutes: string) => {
        if (!nextDate) {
          onChange('')
          return
        }

        const clamped = clampLocalTime(
          nextDate,
          nextHours,
          nextMinutes,
          minDate
      ...
      >
        <div className='flex flex-col gap-3'>
          ...
          <div className='flex items-center gap-2'>
              ...
              <SelectContent className='max-h-56'>
                {MINUTE_OPTIONS.map((minute) => (
                  <SelectItem
                    key={minute}
                    value={minute}
                    disabled={isMinuteOptionDisabled(
                      minute,
                      hours,
                      date,
                      minDate
            ...
            <span className='ps-1 text-xs font-medium tracking-wide text-muted-foreground'>
              hs
            </span>
          </div>
        </div>
