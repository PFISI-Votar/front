...
import { pausarEleccion } from '../api/eleccion-api'

export const usePausarEleccion = (idEleccion: number) => {
  ...
  const runInBackground = useCallback(
    (razon: string) => {
      ...
      runBackgroundOperation({
        loadingMessage: 'Procesando confirmación de pausa...',
        successMessage: (estado) =>
          estado?.ejecutada
            ? 'Comicio pausado correctamente.'
            : `Confirmación registrada (${estado?.confirmaciones}/${estado?.requeridas}). Falta que otra autoridad PAUSER confirme.`,
        errorTitle: 'No se pudo pausar el comicio',
        operation: () => pausarEleccion(idEleccion, razon),
